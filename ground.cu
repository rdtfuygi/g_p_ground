...
void ground::data(std::vector<float>& 数据)
{
	...
	数据.push_back(float(door[0]));
	数据.push_back(float(door[1]));
	数据.push_back(area());
}
...
float 奖励函数(ground 场地, std::vector<building>& 建筑, bool& reset)
{
	...
	for (int i = 0; i < 建筑.size(); i++)
	{
		...
		分数 += 1 - float(pow(1 - 面积 / 建筑[i].target_area, 2)) * 面积_权重;
		printf("%.3f,", 面积 / 建筑[i].target_area);

		float 周长 = 0;
		for (int j = 0; j < 20; j++)
		{
			float a = fmaxf(fmaxf((建筑[i].site[j].dir * 建筑[i].site[(j + 1) % 20].dir), 0), abs(建筑[i].site[j].dir ^ 建筑[i].site[(j + 1) % 20].dir));
			分数 += (a + float(pow(a, 16))) / 2 / 20 * 平直角_权重;
